...
import { displayTitle, evidenceSummary, ledgerHash } from '../lib/research'
...
export function Ledger({ data }) {
  ...
  const { records: saved, error: researchError } = useResearch()
  const rows = open.filter(c => !params.status || c.status === params.status)
    ...
    .filter(c => !params.q || [c.id, c.short_title, c.name, c.who, c.problem, c.industry].join(' ').toLowerCase().includes(params.q.trim().toLowerCase()))
  ...
  return <div className="pt-2">
    <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
      <div><h1 className="text-2xl font-semibold tracking-tight">机会库</h1></div>
    ...
    <div className="mb-4 flex flex-wrap gap-2">
      ...
      <details className="filter-popover"><summary><Filter className="size-3.5" aria-hidden="true" />筛选{Object.entries(params).filter(([key, value]) => !['q', 'selected'].includes(key) && value).length > 0 && <span className="text-accent"> · 已设</span>}</summary><div className="filter-popover-body">
    ...
    <div className="mb-3 flex flex-wrap items-center gap-3 text-xs text-muted"><p role="status">{rows.length} 个机会</p>{Object.entries(params).some(([key, value]) => key !== 'selected' && value) && <a className="text-accent underline" href="#/ledger">清除筛选</a>}</div>
    {params.lens === 'saved' && researchError && <p role="alert" className="mb-4 text-sm text-accent">{researchError}</p>}
    {rows.length ? <div className="desk">
      <div className="desk-list"><ul>{rows.map(c => {
        ...
        return <li key={c.id}><a className="desk-row" aria-current={selected?.id === c.id ? 'true' : undefined} href={fullLink(c)} onClick={e => {
          if (window.matchMedia('(min-width: 1280px)').matches && !e.metaKey && !e.ctrlKey && !e.shiftKey && e.button === 0) { e.preventDefault(); change('selected', c.id) }
        }}>
          <div className="mb-2 flex items-center justify-between gap-2"><span className="eyebrow">{c.id}</span><Tag>{c.industry || '跨行业'}</Tag></div>
          <h2 className="text-base leading-relaxed font-semibold" title={c.name}>{displayTitle(c)}</h2>
      ...
      <article className="desk-detail" key={selected.id} aria-label="选中机会详情">
        ...
        <ClusterDetail cluster={selected} rubric={data.rubric} headingLevel={3} />
      </article>
    </div> : <div className="py-16 text-center"><h2>暂无匹配机会</h2><a className="filter-link mt-5 inline-block" href="#/ledger">清除筛选</a></div>}
  </div>
}
