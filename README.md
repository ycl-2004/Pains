...
网站已采用 YC 酒红 / 丹宁蓝 / 纸色视觉：固定侧栏、Today 研究首页、桌面双栏证据底稿、手机独立详情，以及本地验证记录。筛选可通过 URL 分享；验证记录仅保存在当前浏览器，可导出/导入 JSON 备份，不会同步或改变公共评分。设计与数据契约见 [研究工作台说明](docs/research-desk.md)，商业判断校准见 [评估样本](evals/README.md)。
...
- **数据保留是有上限的。** 每次运行会自动清理 14 天前的原始抓取、90 天前的运行报告、180 天前的台账快照和复查记录；归档机会保留 365 天，信号保留 90 天，单个机会最多保留 52 次评分历史。保留期限可用 `RADAR_*_RETENTION_DAYS` 和 `RADAR_MAX_SCORE_HISTORY` 覆盖。
